/* toggle between the data and data */
function toggleDataData(event, data) {

	var $ev = $(event.target);
	var $data = $ev.parents('.bootcards-data');
	...
	if ($data.length>0) {
		...
		$data.fadeOut( 'fast', function()  {
			$data
				.siblings('.bootcards-data')
					.fadeIn('fast', function() {
						if (typeof data != 'undefined' && data != null) { data.redraw();}
					});
		});
			...
			$(window)
		        ...
		        .on( 'resize', function() {
		        	setTimeout( function() {
		                ...
		                if (dataSalesProductType !== null) { dataSalesProductType.redraw(); }
		                if (closedSalesData !== null) { closedSalesData.redraw(); }
		                if (dbSizeData !== null) { dbSizeData.redraw(); }
		                if (barDataClosedSales !== null) { barDataClosedSales.redraw(); }
		            } , 250);
		        } );
