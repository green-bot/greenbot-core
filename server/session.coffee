...
module.exports = ->
  ...
  events.on 'telnet:ingress', (msg) ->
    info 'Ingress message for telnet ' + JSON.stringify msg
    Session.findOrCreate msg, (dst, txt) ->
      info 'Egress message for telnet ' + txt
      events.emit "telnet:egress:telnet", txt

  events.on 'slack:ingress', (msg) ->
    Session.findOrCreate msg, (dst, txt) ->
      events.emit "slack:egress", dst, txt

  events.hear /(.*)/i, (hubotMsg) ->
    ...
    Session.findOrCreate msg, (src, txt) ->
      user = events.brain.userForId dst, name: src
      events.send user, txt
